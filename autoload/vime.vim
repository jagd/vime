...
function! VimeComplete(findstart, base)
	  if a:findstart
        ...
        while bd1 < bd2
            let i = (bd1+bd2) /2
            let code = g:vimetable[i*2] " ind=/2 and ind*2  cancelled
            if a:base < code
                let bd2 = i - 1
            elseif a:base > code
                let bd1 = i + 1 
        ...
        "                         that there is no exact match)
        "      or found one of the solutions at bd1
        let range1 = bd1
        while range1 > 0
            let range1 -= 1
            if g:vimetable[range1*2] != code
        ...
        while i >= 0 && range2 <= s:numtable
                   \ && strpart(g:vimetable[range2*2], 0, codelen) == code
            let i -= 1
            let range2 += 1
        endwhile
